...
def build_report_package(
    ...
    frame_descriptions: list[str | None] | None = None,
) -> dict[str, Any]:
    desc_list = frame_descriptions or [None] * len(frames)
    if len(desc_list) < len(frames):
        desc_list = list(desc_list) + [None] * (len(frames) - len(desc_list))
    elif len(desc_list) > len(frames):
        desc_list = desc_list[: len(frames)]

    frame_rows = []
    for frame, description in zip(frames, desc_list):
    ...
    report = {
        ...
        "stats": {
            ...
            "frame_fps": fps,
            "frame_descriptions_count": sum(1 for r in frame_rows if r.get("description")),
        },
        "frames": frame_rows,
...
def _write_markdown_report(path: Path, report: dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append("## Frame descriptions (OpenRouter vision)")
    lines.append("")
    if report["stats"].get("frame_descriptions_count", 0) == 0 and report["frames"]:
        lines.append(
            "*No vision descriptions were stored. Set `OPENROUTER_API_KEY` and optional `OPENROUTER_MODEL` "
            "(default `google/gemini-2.5-flash`) to describe frames.*"
        )
        lines.append("")
    for row in report["frames"][:40]:
        desc = row.get("description") or "(no description)"
        lines.append(f"- **{row['timestamp']}** — {desc}")
        lines.append(f"  - file: `{row['path']}`")
    if len(report["frames"]) > 40:
...
def _write_agent_context(path: Path, report: dict[str, Any]) -> None:
    ...
    for row in report["frames"]:
        ...
        if desc:
            lines.append(f"- t={row['timestamp']} desc={desc} file={row['path']}")
        else:
